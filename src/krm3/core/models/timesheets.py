...
class TimesheetSubmissionManager(models.Manager):
    ...
    def get_closed_in_period(
        ...
        if isinstance(resources, Resource):
            resources = [resources]
        return self.filter(
            period__overlap=KTDateRange.from_start_end(from_date, to_date), closed=True, resource__in=resources
        )
...
class TimesheetSubmission(models.Model):
    ...
    def calculate_timesheet(self) -> dict:
        ...
        lower = KTDay(self.period.lower).date
        upper = KTDay(self.period.upper).date
...
class ExtraHolidayManager(models.Manager):
    def unnested(self) -> QuerySet[ExtraHoliday]:
        """Unnest the ExtraHoliday country codes."""
        return self.get_queryset().annotate(country_code=Unnest('country_codes'))

    def resolve(self, from_date: datetime.date, to_date: datetime.date) -> dict[str, ExtraHoliday]:
        """Unnest the ExtraHoliday country codes and return a dict mapping each day in period to a country code."""
        search_range = KTDateRange.from_start_end(from_date, to_date)
        result = {}

        for eh in (
            self.get_queryset()
            .filter(period__overlap=search_range)
            .annotate(
                intersection=DateRangeIntersection(F('period'), search_range), country_code=Unnest('country_codes')
            )
        ):
            for day in KTDateRange(eh.intersection):
                result.setdefault(day, []).append(eh.country_code)
        return result
