...
class KrmDay:
    ...
    def __init__(
        self,
        day: MaybeKrmDayType = None,
        country_calendar_code: str = None,
        sundays_as_holiday: bool | None = True,
        extra_holiday_codes: list[str] | None = None,
        **kwargs,
    ) -> None:
        self.set_country_calendar(country_calendar_code, sundays_as_holiday, extra_holiday_codes)
    ...
    def set_country_calendar(
        self,
        country_calendar_code: str = None,
        sundays_as_holiday: bool | None = True,
        extra_holiday_codes: list[str] | None = None,
    ) -> KrmDay:
        """Set country calendar-specific attributes."""
        self.country_calendar_code = country_calendar_code or settings.HOLIDAYS_CALENDAR
        self.sundays_as_holiday = sundays_as_holiday if sundays_as_holiday is not None else True
        self.extra_holiday_codes = extra_holiday_codes
        return self
    ...
    def is_extra_holiday(self) -> bool:
        from krm3.utils.extra_holidays import extra_holidays  # imported lazily to avoid circular import

        codes = self.extra_holiday_codes
        if codes is None:
            codes = extra_holidays.get(self.month, self.year).get(self, [])
        return self.country_calendar_code in codes or (
            len(self.country_calendar_code) > 2 and self.country_calendar_code[:2] in codes
        )

    @property
    def is_holiday(self) -> bool:
        if self.is_extra_holiday:
            return True
        if self.sundays_as_holiday:
            return not get_country_holidays(country_calendar_code=self.country_calendar_code).is_working_day(self.date)
        return self.date in get_country_holidays(country_calendar_code=self.country_calendar_code)
...
class KrmCalendar(Calendar):
    ...
    def get_work_days(self, from_date: KrmDayType, to_date: KrmDayType) -> Iterator[KrmDay]:
        for day in self.iter_dates(from_date, to_date):
            if not day.is_non_working_day():
                yield day
...
#
#     @staticmethod
#     def from_start_end(start_date: MaybeKrmDayType, end_date: MaybeKrmDayType) -> KrmDateRange:
...
#
#     def intersection(self, other: PeriodType) -> 'KrmDateRange | None':
#         """Return the intersection with the other range, or None if they do not overlap."""
#         other = KrmDateRange(other).as_dates()
#         period = self.as_dates()
#         lower = max(period.lower, other.lower)
#         upper = min(period.upper, other.upper)
#         if lower >= upper:
#             return None
#         return KrmDateRange(
...
#         for x in range((self.upper - self.lower).days + (1 if self.upper_inc else 0)):
#             yield dt + x
#
#     def __str__(self) -> str:
