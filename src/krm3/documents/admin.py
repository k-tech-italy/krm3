...
class Krm3DocumentAdmin(ExtraButtonsMixin, DocumentAdmin):
    ...
    def import_payslips(self, request: 'HttpRequest') -> None:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                messages.success(
                    request, _l('Successfully imported {num} payslips.'.format(num=len(form._resources_found)))
                )
            else:
                messages.error(request, _l('Error. Found {num} payslips.'.format(num=len(form._resources_found))))
