...
from testutils.factories import ResourceFactory, SpecialLeaveReasonFactory, TimeEntryFactory, TimesheetSubmissionFactory
...
def test_sick_leave_protocol_number_is_visible_in_report_while_not_submitted(
    ...
    TimeEntryFactory(
        date=datetime.date(2026, 3, 2), day_shift_hours=0, sick_hours=8, protocol_number='12', resource=resource
    )
    TimeEntryFactory(date=datetime.date(2026, 3, 3), day_shift_hours=0, sick_hours=8, resource=resource)
...
def test_sick_leave_protocol_number_is_visible_in_report_while_submitted(
    ...
    resource = ResourceFactory(user=regular_user)
    TimeEntryFactory(
        date=datetime.date(2026, 3, 1), day_shift_hours=0, sick_hours=8, protocol_number='12', resource=resource
    )
    TimeEntryFactory(date=datetime.date(2026, 3, 3), day_shift_hours=0, sick_hours=8, resource=resource)
    TimesheetSubmissionFactory(
        resource=resource, closed=True, period=(datetime.date(2026, 3, 1), datetime.date(2026, 3, 31))
    )
    browser.login_as_user(regular_user)
