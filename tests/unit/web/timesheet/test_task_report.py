...
class TestTimesheetTaskReport:
    ...
    def setup_task_report_test_data(self, admin_user):
...
def test_task_report_view_next_previous_month(admin_client, month, expected_result):
    response = admin_client.get(reverse('task-report-month', args=[month]))
    _assert_homepage_content(response)
    assert response.status_code == 200
