...
def test_availability_view_current_month(admin_client):
...
def test_availability_view_filtered_by_project(admin_client):
    ...
    url = reverse('availability')
    response = admin_client.get(f'{url}?project={project.id}')
    _assert_homepage_content(response)
...
def test_availability_view_next_previous_month(admin_client, month, expected_result):
    response = admin_client.get(reverse('availability-report-month', args=[month]))
    _assert_homepage_content(response)
    assert response.status_code == 200
