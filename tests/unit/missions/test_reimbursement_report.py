...
def test_reimbursement_report_data_marker(expenses):
    ...
    for i, (expense_set, outcome) in enumerate(expenses):
        prepare_data(expense_set, i, reimbursments_per_resource, resource_cache)
...
def prepare_data(expense_set, i, reimbursments_per_resource, resource_cache):
    for mission, expense_data in expense_set.items():
        ...
        mission_obj = MissionFactory(year=2025, number=int(m_number), resource=resource, from_date=m_from, to_date=m_to)
