...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(dt('2031-01-01'), id='closed-ended'),
    ],
)
def test_task_with_contract(end_date, project_2020_open):
    contract: Contract = ContractFactory(period=(dt('2020-01-01'), end_date))
    ...
    form = TaskForm(
        ...
        | {
            ...
            'period_1': end_date.strftime('%Y-%m-%d') if end_date else "",
        }
    )
...
def test_task_spanning_contracts(project_2020_open):
    c1: 'Contract' = ContractFactory(period=(dt('20200101'), dt('20220101')))
    ContractFactory(resource=c1.resource, period=(dt('20220101'), dt('20250101')))
    ContractFactory(resource=c1.resource, period=(dt('20250101'), None))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            [
                [dt('2020-01-01'), dt('2024-01-01')],
                [dt('2024-04-01'), dt('2026-01-01')],
            ],
            id='gap-middle',
        ),
    ],
...
@pytest.mark.parametrize(
    'start_date, end_date',
    [
        pytest.param(dt('2020-01-02'), None),
        pytest.param(dt('2020-01-02'), dt('2020-01-04')),
        pytest.param(dt('2020-01-01'), dt('2020-01-03')),
    ],
)
...
def test_orphan_te_check(orphan_scenario):
    ...
    form = TaskForm(instance=task, data=data | {'color': '#FFFFFF', 'period_0': str(KTDay(period.lower) + 2)})
