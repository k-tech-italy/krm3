...
def orphan_scenario():
    ...
    contract = ContractFactory(resource=task.resource, period=(task.start_date, task.end_date))
    contract2 = ContractFactory(resource=task2.resource, period=(task2.start_date, task2.end_date))

    WorkScheduleFactory(contract=contract)
    WorkScheduleFactory(contract=contract2)
